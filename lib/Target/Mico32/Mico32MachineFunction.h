//===-- Mico32MachineFunctionInfo.h - Private data ----------------*- C++ -*-=//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares the Mico32 specific subclass of MachineFunctionInfo.
//
//===----------------------------------------------------------------------===//

#ifndef MICO32_MACHINE_FUNCTION_INFO_H
#define MICO32_MACHINE_FUNCTION_INFO_H
...
private:
  ...
  /// Mico32FIHolder - Holds a FrameIndex and it's Stack Pointer Offset
  struct Mico32FIHolder {

    int FI;
    int SPOffset;

    Mico32FIHolder(int FrameIndex, int StackPointerOffset)
  ...
  Mico32FIHolder GPHolder;

  /// On LowerFormalArguments the stack size is unknown, so the Stack
  /// Pointer Offset calculation of "not in register arguments" must be
  /// postponed to emitPrologue.
  SmallVector<Mico32FIHolder, 16> FnLoadArgs;
  bool HasLoadArgs;
  ...
  // postponed to emitPrologue.
  SmallVector<Mico32FIHolder, 4> FnStoreVarArgs;
...
public:
  Mico32FunctionInfo(MachineFunction& MF)
  ...
  void recordLoadArgsFI(int FI, int SPOffset) {
    ...
    FnLoadArgs.push_back(Mico32FIHolder(FI, SPOffset));
  }

  void recordStoreVarArgsFI(int FI, int SPOffset) {
    if (!HasStoreVarArgs) HasStoreVarArgs=true;
    FnStoreVarArgs.push_back(Mico32FIHolder(FI, SPOffset));
  }
...
#endif // MICO32_MACHINE_FUNCTION_INFO_H
